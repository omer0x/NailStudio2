/*
  ...
  1. Changes
    - Simplify validation logic to be more robust
    - Block slots immediately for both pending and confirmed appointments
    - Improve consecutive slot checking
    - Add better error messages
    - Fix edge cases with slot availability
...
BEGIN
  ...
  FOR current_slot IN (
    ...
    IF EXISTS (
      ...
      RAISE EXCEPTION 'Time slot % is already booked', current_slot.start_time;
    END IF;
  ...
  -- Verify the slots are truly consecutive
  IF last_valid_start_time != slot_start_time + (interval '30 minutes' * (required_slots - 1)) THEN
    RAISE EXCEPTION 'Time slots are not consecutive';
  END IF;
